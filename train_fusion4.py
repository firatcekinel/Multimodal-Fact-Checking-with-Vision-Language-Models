...
class FusionDataset(Dataset):
    def __init__(self, data, claim_embeddings, claim_image_embeddings, text_embeddings, image_embeddings):
        self.data = data
        self.claim_embeddings = claim_embeddings
        self.claim_image_embeddings = claim_image_embeddings
    ...
    def __getitem__(self, idx):
        ...
        return dict(
            ...
            text=row.Claim,
            claim_embedding=torch.tensor(self.claim_embeddings[idx]).reshape(-1),
            claim_image_embedding=torch.tensor(self.claim_image_embeddings[idx]).reshape(-1),
            text_embedding=torch.tensor(self.text_embeddings[idx]).reshape(-1),
            text_image_embedding=torch.tensor(self.image_embeddings[idx]).reshape(-1),
        )
...
class FusionDataModule(pl.LightningDataModule):
    def __init__(
        ...
        train_claim_embeddings,
        train_claim_image_embeddings,
        val_claim_embeddings,
        val_claim_image_embeddings,
        test_claim_embeddings,
        test_claim_image_embeddings,
        ...
        self.train_claim_embeddings = train_claim_embeddings
        self.train_claim_image_embeddings = train_claim_image_embeddings
        self.val_claim_embeddings = val_claim_embeddings
        self.val_claim_image_embeddings = val_claim_image_embeddings
        self.test_claim_embeddings = test_claim_embeddings
        self.test_claim_image_embeddings = test_claim_image_embeddings
    ...
    def setup(self, stage=None):
        self.train_dataset = FusionDataset(
            self.train_df,
            self.train_claim_embeddings,
            self.train_claim_image_embeddings,
            self.train_text_embeddings,
            self.train_image_embeddings
        )
        self.val_dataset = FusionDataset(
            self.val_df,
            self.val_claim_embeddings,
            self.val_claim_image_embeddings,
            self.val_text_embeddings,
            self.val_image_embeddings
        )
        self.test_dataset = FusionDataset(
            self.test_df,
            self.test_claim_embeddings,
            self.test_claim_image_embeddings,
...
class FusionModel(pl.LightningModule):
    def __init__(self, input_size1=1024, input_size2=4096, input_size3=1024, input_size4=4096, hidden_size=256, lr=0.01, dropout=0.1, num_classes=3, training_steps=None, warmup_steps=None, class_weights=torch.ones(3).to(device)):
        super().__init__()
        self.embedding1_fc = torch.nn.Linear(input_size1, hidden_size)
        self.embedding2_fc = torch.nn.Linear(input_size2, hidden_size)
        self.embedding3_fc = torch.nn.Linear(input_size3, hidden_size)
        self.embedding4_fc = torch.nn.Linear(input_size4, hidden_size)
        self.relu = torch.nn.ReLU()
        self.fc1 = torch.nn.Linear(hidden_size*4, hidden_size//2)
    ...
    def forward(self, emb1, emb2, emb3, emb4):
        ...
        out3 = self.dropout(self.relu(self.embedding3_fc(emb3)))
        out4 = self.dropout(self.relu(self.embedding4_fc(emb4)))
    ...
    def training_step(self, batch, batch_size):
        ...
        text_image_embedding = batch["text_image_embedding"]

        claim_embedding = batch["claim_embedding"]
        claim_image_embedding = batch["claim_image_embedding"]

        with torch.autocast(device_type="cuda", dtype=torch.float16):
            outputs = self(claim_image_embedding, claim_embedding, text_image_embedding, text_embedding)
    ...
    def test_step(self, batch, batch_size):
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            outputs = self(batch["claim_image_embedding"], batch["claim_embedding"] ,batch["text_image_embedding"], batch["text_embedding"])
...
def trainModel(params):
    ...
    data_module = FusionDataModule(train_df, val_df, test_df, 
                                   claim_embeddings1, claim_img_embeddings1,
                               claim_embeddings2, claim_img_embeddings2,
                               claim_embeddings3, claim_img_embeddings3,
    ...
    model = FusionModel(input_size1, input_size2, input_size3, input_size4, params["hidden_size"], params["lr"], params["dropout"], num_classes, warmup_steps, total_training_steps, class_weights)
    ...
    for batch in data_module.test_dataloader():
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            outputs = model(batch["claim_image_embedding"].to(device), 
                            batch["claim_embedding"].to(device), 
                            batch["text_image_embedding"].to(device), 
...
parser.add_argument('--isclaim', default=False, type=bool, help='claim or evidence')
parser.add_argument('--train_claim_image_file', default='image/train_image_embeddings.npy', type=str, help='train_image_file')
parser.add_argument('--val_claim_image_file', default='image/val_image_embeddings.npy', type=str, help='val_image_file')
parser.add_argument('--test_claim_image_file', default='image/test_image_embeddings.npy', type=str, help='test_image_file')
parser.add_argument('--train_claim_file', default='claim/train_text_embeddings.npy', type=str, help='train_text_file')
parser.add_argument('--val_claim_file', default='claim/val_text_embeddings.npy', type=str, help='val_text_file')
parser.add_argument('--test_claim_file', default='claim/test_text_embeddings.npy', type=str, help='test_text_file')
parser.add_argument('--train_text_image_file', default='image/train_image_embeddings.npy', type=str, help='train_image_file')
parser.add_argument('--val_text_image_file', default='image/val_image_embeddings.npy', type=str, help='val_image_file')
parser.add_argument('--test_text_image_file', default='image/test_image_embeddings.npy', type=str, help='test_image_file')
parser.add_argument('--train_text_file', default='text/train_text_embeddings.npy', type=str, help='train_text_file')
parser.add_argument('--val_text_file', default='text/val_text_embeddings.npy', type=str, help='val_text_file')
parser.add_argument('--test_text_file', default='text/test_text_embeddings.npy', type=str, help='test_text_file')
parser.add_argument('--wandb_proj_name', default='fusion43_text+image', type=str, help='wandb proj name')
...
# Load embeddings
claim_embeddings1 = np.load(embedding_path + args.train_claim_file)
claim_embeddings2 = np.load(embedding_path + args.val_claim_file)
claim_embeddings3 = np.load(embedding_path + args.test_claim_file)

text_embeddings1 = np.load(embedding_path + args.train_text_file)
text_embeddings2 = np.load(embedding_path + args.val_text_file)
text_embeddings3 = np.load(embedding_path + args.test_text_file, allow_pickle=True)

claim_img_embeddings1 = np.load(embedding_path + args.train_claim_image_file)
claim_img_embeddings2 = np.load(embedding_path + args.val_claim_image_file)
claim_img_embeddings3 = np.load(embedding_path + args.test_claim_image_file)

img_embeddings1 = np.load(embedding_path + args.train_text_image_file)
img_embeddings2 = np.load(embedding_path + args.val_text_image_file)
img_embeddings3 = np.load(embedding_path + args.test_text_image_file)
...
if args.dataset == "factify2": # test set has no labels, use val set for testing
    ...
    claim_embeddings3 = claim_embeddings2
    claim_embeddings2 = claim_embeddings1[indices]
    mask = np.ones(len(claim_embeddings1), dtype=bool)
    mask[indices] = False
    claim_embeddings1 = claim_embeddings1[mask]

    claim_img_embeddings3 = claim_img_embeddings2
    claim_img_embeddings2 = claim_img_embeddings1[indices]
    mask = np.ones(len(claim_img_embeddings1), dtype=bool)
    mask[indices] = False
    claim_img_embeddings1 = claim_img_embeddings1[mask]

input_size1 = claim_img_embeddings1[0].shape[-1] # image embedding
input_size2 = claim_embeddings1[0].shape[-1] # text embedding

input_size3 = img_embeddings1[0].shape[-1] # image embedding
input_size4 = text_embeddings1[0].shape[-1] # text embedding
